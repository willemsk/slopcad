...
export function exportPageToSVG(page: Page, unitSystem: UnitSystem): string {
  ...
  elements.push(`<rect x="${minX}" y="${minY}" width="${width}" height="${height}" fill="#1e2028" />`);
  ...
  for (const ent of page.entities) {
    if (ent.type === 'stairs') {
      ...
      const textAngleDeg = (angRad > Math.PI / 2 || angRad < -Math.PI / 2) ? (angRad + Math.PI) * (180 / Math.PI) : angRad * (180 / Math.PI);
...
export function downloadSVGFile(page: Page, unitSystem: UnitSystem) {
  ...
  const filename = page.name.trim().toLowerCase().replace(/\s+/g, '_') || 'floor_plan';
  a.href = url;
