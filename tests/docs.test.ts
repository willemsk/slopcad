...
describe('Documentation Build', () => {
  it('should successfully build the MkDocs site without errors or broken links', () => {
    ...
    try {
      ...
      throw new Error(`Failed to generate API docs: ${e.message}\n${e.stdout?.toString() || ''}\n${e.stderr?.toString() || ''}`);
    }

    // 2. Determine mkdocs executable path
    // Check for local venv first (Windows then Unix)
    const winVenvPath = path.join(process.cwd(), 'venv', 'Scripts', 'mkdocs.exe');
    ...
    try {
      ...
      throw new Error(`MkDocs build failed or has broken links: ${e.message}\n${e.stdout?.toString() || ''}\n${e.stderr?.toString() || ''}`);
    }
