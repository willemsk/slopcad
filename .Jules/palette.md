...
## 2026-07-06 - Missing Keyboard and Screen Reader Support on Custom Accordion Headers\n**Learning:** The property panel category headers (e.g., in PageProperties, GeneralProperties, etc.) were implemented as `div` elements with `onClick` handlers, which made them inaccessible to keyboard users (no focus, no Enter/Space to toggle) and invisible as interactive elements to screen readers (no `role="button"`, `aria-expanded`, or `aria-label`).\n**Action:** When creating custom toggleable headers or accordions, always apply `role="button"`, `tabIndex={0}`, `aria-expanded`, `aria-label`, and an `onKeyDown` handler to support Enter/Space toggling to ensure full accessibility.
