...
## 2026-07-20 - Property Category Headers Keyboard Accessibility
**Learning:** Accordion-style headers used for categories in the properties panel (`PageSettings`, `General`, `Geometry`, etc.) were implemented as simple `<div>` elements with an `onClick` handler. Because they lacked `tabIndex`, they were completely inaccessible to keyboard users, making it impossible to navigate the properties panel without a mouse. Furthermore, they lacked `role="button"` and `aria-expanded`, so screen readers did not identify them as toggleable controls.
**Action:** When implementing custom accordion or toggleable UI elements with `<div>` or `<span>`, always apply `tabIndex={0}`, `role="button"`, and handle `onKeyDown` for 'Enter'/'Space' to ensure they are fully operable by keyboard and assistive technologies. Additionally, add a clear `:focus-visible` state in CSS.
